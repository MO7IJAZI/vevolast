...
import { db } from "./db";
import { users, invitations, passwordResets, clientUsers } from "../shared/schema.js";
...
export function registerAuthRoutes(app) {
    ...
    app.post("/api/auth/invite", requireAdmin, async (req, res) => {
        try {
            ...
            const requestedPermissions = Array.isArray(permissions)
                ? permissions.filter((p) => typeof p === "string" && p.length > 0)
                : [];
            const resolvedPermissions = requestedPermissions.length > 0 ? requestedPermissions : roleDefaultPermissions[resolvedRole] || [];
            ...
            const emailSent = await sendInvitationEmail(email.toLowerCase(), name, token, resolvedRole);
            res.json({
    ...
    app.get("/api/client-users/by-client/:clientId", requireAdmin, async (req, res) => {
        try {
            ...
            const [clientUser] = await db.select({
                ...
                createdAt: clientUsers.createdAt,
                lastLogin: clientUsers.lastLogin,
            }).from(clientUsers).where(eq(clientUsers.clientId, clientId)).limit(1);
            if (!clientUser) {
    ...
    app.patch("/api/client-users/:id/toggle-active", requireAdmin, async (req, res) => {
        try {
            ...
            await db.update(clientUsers).set({ isActive: !clientUser.isActive }).where(eq(clientUsers.id, id));
