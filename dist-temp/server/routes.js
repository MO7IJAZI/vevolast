...
import { goalFormSchema, insertTransactionSchema, insertClientPaymentSchema, insertPayrollPaymentSchema, insertEmployeeSalarySchema, insertCalendarEventSchema, insertNotificationSchema, insertClientSchema, insertLeadSchema, insertClientServiceSchema, insertMainPackageSchema, insertSubPackageSchema, insertInvoiceSchema, insertEmployeeSchema, insertSystemSettingsSchema,
// attendance,
...
export async function registerRoutes(httpServer, app) {
    ...
    app.patch("/api/main-packages/:id", requireAdmin, async (req, res) => {
        try {
            ...
            const pkg = await storage.updateMainPackage(req.params.id, validated);
            if (!pkg) {
    ...
    app.delete("/api/main-packages/:id", requireAdmin, async (req, res) => {
        try {
            const deleted = await storage.deleteMainPackage(req.params.id);
            if (!deleted) {
                return res.status(404).json({ error: "Package not found" });
            }
            res.status(204).send();
    ...
    app.get("/api/sub-packages", async (req, res) => {
        try {
            ...
            const packages = await storage.getSubPackages(mainPackageId);
    ...
    app.patch("/api/sub-packages/:id", requireAdmin, async (req, res) => {
        try {
            ...
            const pkg = await storage.updateSubPackage(req.params.id, validated);
            if (!pkg) {
                return res.status(404).json({ error: "Package not found" });
            }
            res.json(pkg);
    ...
    app.delete("/api/sub-packages/:id", requireAdmin, async (req, res) => {
        try {
            const deleted = await storage.deleteSubPackage(req.params.id);
            if (!deleted) {
                return res.status(404).json({ error: "Package not found" });
            }
            res.status(204).send();
    ...
    app.patch("/api/employees/:id", async (req, res) => {
        try {
            ...
            if (!isAdmin) {
                ...
                const { salaryType, salaryAmount, rate, rateType, salaryCurrency, salaryNotes, // Salary info
    ...
    app.patch("/api/transactions/:id", requireAdmin, async (req, res) => {
        try {
            ...
            const updated = await storage.updateTransaction(req.params.id, validated);
            if (!updated) {
    ...
    app.post("/api/client-payments", requireAdmin, async (req, res) => {
        try {
            ...
            await storage.createTransaction({
                ...
                type: "income",
                category: "other", // Default category, could be refined
                ...
                notes: validated.notes || "Auto-generated from client payment",
            });
    ...
    app.patch("/api/client-payments/:id", requireAdmin, async (req, res) => {
        try {
            ...
            const updated = await storage.updateClientPayment(req.params.id, validated);
            if (!updated) {
    ...
    app.patch("/api/payroll-payments/:id", requireAdmin, async (req, res) => {
        try {
            ...
            const updated = await storage.updatePayrollPayment(req.params.id, validated);
            if (!updated) {
    ...
    app.delete("/api/payroll-payments/:id", requireAdmin, async (req, res) => {
        try {
            const deleted = await storage.deletePayrollPayment(req.params.id);
            if (!deleted) {
                return res.status(404).json({ error: "Payroll payment not found" });
            }
            res.status(204).send();
